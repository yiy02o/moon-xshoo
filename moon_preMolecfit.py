...
def median_2DSLIT(directory, ARM, plot=False, axes=None):
    ...
    for folder in os.listdir(directory):
        if folder == 'README':
            ...
            for file in os.listdir(directory + '/' + folder):
                if 'SCI_SLIT_MERGE2D_' + ARM in file:
    ...
    if plot:
        wv = bin_table['WAVE']
        axes.plot(wv, median_flux, lw=0.5, label=date_obs)
        axs.fill_between(wv, median_flux - std_flux, median_flux + std_flux, alpha=0.3)
...
def medianALL_per_Region(directory, ARM, axes, color='b', normalized=False, plot=True):
    ...
    idx_avoid = 700
    idx_min, idx_max = -2000, -1000  # rango para normalizar
    ...
    if normalized:
        ...
        if plot:
            ...
            return np.array([np.asarray(wvs_list[0][idx_avoid:]), flux_norm[idx_avoid:]])
        ...
        if plot:
            ...
            # To delimit a zone to normalize
            # axes.axvline(w_min, color='b')
            # axes.axvline(w_max, color='b')
            # axes.axvspan(w_min, w_max, alpha=0.5, color='b')
            ...
            return np.array([np.asarray(wvs_list[0][idx_avoid:]), median_ALL[idx_avoid:]])
...
def residues(path_arr, plot=False):
    ...
    if plot:
        ...
        for w, res, arm, ax in zip(wave_per_band, res_per_band, ARMS, axs):
        ...
        return wave_per_band, res_per_band
...
print(n_subdir)
fig, axs = plt.subplots(4, 5, figsize=(25, 10), layout="constrained")
for f, ax in zip(os.listdir(path_Highlands), fig.axes):
    median_2DSLIT(path_Highlands + f, 'UVB', plot=True, axes=ax)
plt.show()
